...
//
// adc_postgres_query.js
// Query functions specific to ADC API for Postgres.
//
// VDJServer
// https://vdjserver.org
//
...
var ADCPostgresQuery = {};
module.exports = ADCPostgresQuery;
...
var pgIO = require('vdj-tapis-js/adc_pgIO');

ADCPostgresQuery.formatField = function(name, stay_jsonb = false) {
...
ADCPostgresQuery.constructWhereClause = function(filter, error, values) {
    var context = 'ADCPostgresQuery.constructWhereClause';
    ...
    case '=': {
        ...
        let content_field = ADCPostgresQuery.formatField(content['field']);
    ...
    case 'contains': {
        ...
        let content_field = ADCPostgresQuery.formatField(content['field'], true);
        if (content_field == null) {
            error['message'] = 'invalid field: ' + content['field'];
            return null;
        }
    ...
    case 'and': {
        ...
        for (let i = 0; i < content.length; ++i) {
            let exp = ADCPostgresQuery.constructWhereClause(content[i], error, values);
